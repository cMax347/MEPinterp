...
!
...
#endif
	write(*,'(a,i5,a,a,a)')	'[#',mpi_id,';main/',cTIME(time()),']:	welcome to mepInterp'
	!
	!
    !BODY
    if( init_parameters()	) then
		...
		write(*,'(a,i5,a)')		'[#',mpi_id,';main]: input file not found, by'
	end if
	!
	!
	!FINALIZE
...
#endif
	write(*,'(a,i5,a,a,a)')	'[#',mpi_id,';main/',cTIME(time()),']:	all done, by by'	
	!
	!
	stop
end program 